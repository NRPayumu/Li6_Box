...
PrimaryGeneratorAction::PrimaryGeneratorAction(G4String datFileName)
{
	...
	if(Particle_Name=="fast_neutron")particle = particleTable->FindParticle("neutron");
	fParticleGun -> SetParticleDefinition(particle);
...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  fParticleGun -> SetParticleEnergy(0.025 *MeV);
	...
	*/
	...
	fParticleGun -> SetParticlePosition(G4ThreeVector(0, 0, -GdLSSizeZ/2));
	fParticleGun -> SetParticleMomentumDirection(G4ThreeVector(0, 0, GdLSSizeZ/2));
